    ...
    <div class="col-md-7 well right-column">
        ...
        <div class="btn-group btn-group-justified" role="group">
            ...
            <div class="btn-group" role="group">
              <a href="<?= $this->Url->build(['controller' => 'Management', 'action' => 'medication'])?>" role="button" class="btn btn-primary">Medication</a>
            </div>
            <div class="btn-group" role="group">
                    ...
                    <div class="col-md-12">
                        ...
                        ?>
                            ...
                            <div class="row">
                                <div class="col-md-2">
                                    <?php
                                        if (($q->medication_group == 'x') && in_array($q->id, $already_ordered)) {
                                            $no_selection = true;
                                        }

                                        if (!in_array($q->id, $already_ordered)) {
                                    ?>
                                            <?php if ($no_selection || (($q->medication_group != -1 && $q->medication_group != 'x') && in_array($q->id, $groups[$q->medication_group]['ids']) && count(array_intersect($already_ordered, $groups[$q->medication_group]['ids'])) > 0)) {?>
                                                <button id="question-button-<?=$q->id?>" type="button" class="btn btn-default btn-md hidden red" onClick="javascript:clickButton('<?=$q->id?>', 'management_medication'<?php if ($q->medication_group != -1) { echo ', \''.$q->medication_group.'\''; }?>);">Delete</button>
                                            <?php } else {?>
                                                <button id="question-button-<?=$q->id?>" type="button" class="btn btn-default btn-md" onClick="javascript:clickButton('<?=$q->id?>', 'management_medication'<?php if ($q->medication_group != -1) { echo ', \''.$q->medication_group.'\''; }?>);">Select</button>
                                            <?}?>
                                    <?php
                                        } else {
                                    ?>
                                            <button id="question-button-<?=$q->id?>" type="button" class="btn btn-default btn-md red" onClick="javascript:clickButton('<?=$q->id?>', 'management_medication'<?php if ($q->medication_group != -1) { echo ', \''.$q->medication_group.'\''; }?>);">Delete</button>
                                    <?php }?>
                                </div>
                                <div class="col-md-10">
                                    <?php
                                        if ($q->medication_group == -1 || $q->medication_group == 'x') {
                                            echo $q->medication_text;
                                        } else {
                                            echo '<span style="color:'.$groups[$q->medication_group]['color'].'">'.$q->medication_text.'</span>';
                                        }
                                    ?>
                                </div>
                            </div>
    ...
    <div class="row">
        ...
        <div class="col-md-2 pull-right">
            <?= $this->Html->link('Next <span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span>', ['controller' => 'management', 'action' => 'referral'], ['class' => 'btn btn-default btn-md pull-right', 'escape' => false])?>
        </div>
    </div>
...
//    });
    ...
    function clickButton(question_id, section_name, group) {
        ...
        if (group == undefined || (group != undefined && confirmed == true)) {
            ...
            var url = "<?php echo $this->Url->build(['controller' => 'ManagementMedications', 'action' => 'select_question']);?>";
