    ...
    <div class="col-md-7 well right-column">
        ...
        <div class="btn-group btn-group-justified" role="group">
            <div class="btn-group" role="group">
              <a href="<?= $this->Url->build(['controller' => 'Management'])?>" role="button" class="btn btn-default">Counseling</a>
            </div>
            <div class="btn-group" role="group">
              <a href="<?= $this->Url->build(['controller' => 'Management', 'action' => 'medication'])?>" role="button" class="btn btn-default">Medication</a>
            </div>
            <div class="btn-group" role="group">
              <a href="<?= $this->Url->build(['controller' => 'Management', 'action' => 'referral'])?>" role="button" class="btn btn-primary">Referral</a>
            </div>
        </div>
        <br />
                    ...
                    <div class="col-md-12">
                        ...
                        ?>
                            ...
                            <div class="row">
                                <div class="col-md-2">
                                    <?php
                                        if (($q->referral_group == 'x') && in_array($q->id, $already_ordered)) {
                                            $no_selection = true;
                                        }

                                        if (!in_array($q->id, $already_ordered)) {
                                    ?>
                                            <?php if ($no_selection || (($q->referral_group != -1 && $q->referral_group != 'x') && in_array($q->id, $groups[$q->referral_group]['ids']) && count(array_intersect($already_ordered, $groups[$q->referral_group]['ids'])) > 0)) {?>
                                                <button id="question-button-<?=$q->id?>" type="button" class="btn btn-default btn-md hidden red" onClick="javascript:clickButton('<?=$q->id?>', 'management_referral'<?php if ($q->referral_group != -1) { echo ', \''.$q->referral_group.'\''; }?>);">Delete</button>
                                            <?php } else {?>
                                                <button id="question-button-<?=$q->id?>" type="button" class="btn btn-default btn-md" onClick="javascript:clickButton('<?=$q->id?>', 'management_referral'<?php if ($q->referral_group != -1) { echo ', \''.$q->referral_group.'\''; }?>);">Select</button>
                                            <?}?>
                                    <?php
                                        } else {
                                    ?>
                                            <button id="question-button-<?=$q->id?>" type="button" class="btn btn-default btn-md red" onClick="javascript:clickButton('<?=$q->id?>', 'management_referral'<?php if ($q->referral_group != -1) { echo ', \''.$q->referral_group.'\''; }?>);">Delete</button>
                                    <?php }?>
                                </div>
                                <div class="col-md-10">
                                    <?php
                                        if ($q->referral_group == -1 || $q->referral_group == 'x') {
                                            echo $q->referral_text;
                                        } else {
                                            echo '<span style="color:'.$groups[$q->referral_group]['color'].'">'.$q->referral_text.'</span>';
                                        }
                                    ?>
                                </div>
                            </div>
    ...
    <div class="row">
        <div class="col-md-10">
            <?= $this->Html->link('<span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span> Previous', ['controller' => 'management', 'action' => 'medication'], ['class' => 'btn btn-default btn-md previous-button', 'escape' => false])?>
        </div>
        <div class="col-md-2 pull-right">
            <?= $this->Html->link('Next <span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span>', ['controller' => 'management', 'action' => 'next_section'], ['class' => 'btn btn-default btn-md pull-right', 'escape' => false])?>
        </div>
    </div>
...
//    });
    ...
    function clickButton(question_id, section_name, group) {
        ...
        if (group == undefined || (group != undefined && confirmed == true)) {
            ...
            var url = "<?php echo $this->Url->build(['controller' => 'ManagementReferrals', 'action' => 'select_question']);?>";
