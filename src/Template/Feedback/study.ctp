...
<script>
    ...
    <div class="col-md-7 well right-column">
        ...
        <div class="btn-group btn-group-justified" role="group">
            <div class="btn-group" role="group">
              <a href="<?= $this->Url->build(['controller' => 'Feedback'])?>" role="button" class="btn btn-primary">Study</a>
            </div>
            <div class="btn-group" role="group">
            ...
            <?if ($hide_billing != 1) {?>
                <div class="btn-group" role="group">
                  <a href="<?= $this->Url->build(['controller' => 'Feedback', 'action' => 'billing'])?>" role="button" class="btn btn-default">Billing</a>
                </div>
            <?}?>
        </div>
        <br />
                        ...
                        <div class="col-md-12">
                            ...
                            <?php if (count($feedback_array['ordered'][$yield_key]) > 0) {?>
                                <?php foreach ($feedback_array['ordered'][$yield_key] as $val) {?>
                                    ...
                                    <br />
                                    <strong>Yield:</strong> <?=$yield_names[$val['study_yield']]?>
                                    <br />
                                    <strong>Result:</strong> <?=$val['result']?>
                                    <br />
                            ...
                            <?php if (count($feedback_array['not_ordered'][$yield_key]) > 0) {?>
                                ...
                                <?php foreach ($feedback_array['not_ordered'][$yield_key] as $val) {?>
                                    ...
                                    <strong>Status:</strong> <?=$val['status']?>
                                    <br />
                                    <strong>Yield:</strong> <?=$yield_names[$val['study_yield_not_ordered']]?>
                                    <br />
                                    <strong>Result:</strong> <?=$val['result']?>
                                    <br />
                                    <strong>Rationale:</strong> <?=$val['rationale']?>
    ...
    <div class="row">
        <div class="col-md-10">
            <?= $this->Html->link('<span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span> Previous', ['controller' => 'billing'], ['class' => 'btn btn-default btn-md previous-button', 'escape' => false])?>
        </div>
        <div class="col-md-2 pull-right">
            <?= $this->Html->link('Next <span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span>', ['controller' => 'feedback', 'action' => 'counseling'], ['class' => 'btn btn-default btn-md pull-right', 'escape' => false])?>
        </div>
    </div>
